...
public class Fixtures {
    ...
    public static MenuBuilder aMenu(){
        return Menu.builder()
                .shop(aShop().build())
                .name("삼겹살 1인세트")
                .description("삼겹살 + 야채세트 + 김치세트")
                .basic(anOptionGroupSpec()
                        .name("기본")
                        .options(java.util.Arrays.asList(anOptionSpec().name("소(250g)").price(Money.wons(12000)).build()))
                        .build())
                .additives(java.util.Arrays.asList(
                        anOptionGroupSpec()
                                .basic(false)
                                .name("맛선택")
                                .options(java.util.Arrays.asList(anOptionSpec().name("매콤 맛").price(Money.wons(1000)).build()))
                                .build()));
    }
    ...
    public static OrderLineItemBuilder anOrderLineItem(){
        return OrderLineItem.builder()
                ...
                .groups(Arrays.asList(
                        ...
                        anOrderOptionGroup()
                        .name("맛선택")
                        .options(Arrays.asList(anOrderOption().name("매콤맛").price(Money.wons(1000)).build()))
                        .build()));
    }
